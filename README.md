...
|---|---|---|
| **1A** - real login | `base.api.ts` fixed (base URL, no baked-in prefix, `credentials: 'include'`), 3 real states (idle/submitting/error), client validation, fixed the `/dashbaord` typo, client-side auth check protecting `/dashboard`, real logout | [`web/lib/services/base.api.ts`](web/lib/services/base.api.ts), [`web/app/login/page.tsx`](web/app/login/page.tsx), [`web/app/dashboard/page.tsx`](web/app/dashboard/page.tsx) |
...
The auth path is client-side: `/dashboard` calls `GET /user/me` on mount and redirects to `/login` if that fails, rather than a server-side check inspecting the cookie's presence. That used to be a Next.js middleware (`proxy.ts`) doing the presence check before any HTML rendered, which worked locally but breaks once frontend and backend are deployed to different domains (Vercel + Render) - the auth cookie belongs to the backend's domain and is never visible to a check running on the frontend's own domain, regardless of cookie settings. The client-side `fetch` to `/user/me` doesn't have that problem, since it's a real cross-origin request that correctly carries the cookie. `/login` deliberately does *not* make this call - it's a public page, so it renders immediately and unconditionally; the only redirect-away-if-signed-in check there is a synchronous read of in-memory state, never a network request a visitor would have to wait on. Express is still the only place the JWT is ever verified - see [docs/04-integration.md](docs/04-integration.md) for where a smart contract would sit in this diagram once that's wired up - it doesn't exist in the running system yet.
...
- **`/dashboard`'s route protection is a client-side `GET /user/me` check on mount** (`app/dashboard/page.tsx`), not a server-side cookie check - there's a brief loading state (`components/session-loading.tsx`) before the redirect fires on an unauthenticated visit. This was originally a Next.js middleware (`proxy.ts`) checking the cookie's presence before any HTML rendered, which is flash-free but only works when frontend and backend share a registrable domain (true in local dev, false once deployed to Vercel + Render on separate domains - the cookie belongs to the backend's domain and a check on the frontend's own domain never sees it). Removed rather than left in as dead code that only functions locally. `/login`, being a public page, does not make this call at all - it renders immediately regardless of auth state, only redirecting away synchronously (no network request, no wait) if a user is already in the in-memory store. Express is still the only place the JWT is ever verified.
